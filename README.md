...
# LTI Service [![GitHub license](https://img.shields.io/badge/license-Apache%20License%202.0-blue.svg?style=flat)](http://www.apache.org/licenses/LICENSE-2.0) [![Build Status](https://img.shields.io/circleci/project/github/coding-eval-platform/lti-service/master.svg)](https://circleci.com/gh/coding-eval-platform/lti-service/tree/master) ![GitHub tag (latest SemVer)](https://img.shields.io/github/tag/coding-eval-platform/lti-service.svg)

Service in charge of providing [LTI 1.3](https://en.wikipedia.org/wiki/Learning_Tools_Interoperability) integration.
This service allows both teachers and students to communicate with the platform through Learning Management Systems (LMS) as, for example, [Blackboard Learn](https://www.blackboard.com/blackboard-learn). Specifically, the service allows teachers to select an exam from within a context in a LMS when building course contents, and student to sit for exams also within the context of a course in the LMS.
...
```
$ git clone https://github.com/coding-eval-platform/lti-service.git
```
or

```
$ wget https://github.com/coding-eval-platform/lti-service/archive/master.zip
```
...
You can do this by changing the ```<project-root>/lti-service-application/src/main/resources/application.yml``` file, in the development section, or by defining the properties through the command line (with ```-Dkey=value``` properties, or with ```--key=value``` properties) when running the application.
...
2. Build the project:
	...
	**Note:** In case you change the ```<project-root>/lti-service-application/src/main/resources/application.yml```, you must build again the project. Otherwise, if you want to change a property on the fly, use command line properties.
...
```
$ export LTI_SERVICE_VERSION=<project-version>
$ java [-Dkey=value properties] -jar <project-root>/lti-service-application/target/lti-service-application-$LTI_SERVICE_VERSION.jar [--key=value properties]
```
...
export LTI_SERVICE_VERSION=<project-version>
java \
	-Dspring.datasource.url=jdbc:postgresql://localhost:5432/coding-eval-platform__lti-service \
	-Dspring.datasource.username=coding-eval-platform__lti-service \
	-Dspring.datasource.password=coding-eval-platform__lti-service \
	-Dspring.kafka.bootstrap-servers=localhost:9092 \
	-jar <project-root>/lti-service-application/target/lti-service-application-$LTI_SERVICE_VERSION.jar \
...
This project includes a ```Dockerfile``` in the ```lti-service-application``` module, together with the [Spotify's dockerfile maven plugin](https://github.com/spotify/dockerfile-maven). 
...
The built Docker image will be ```itbacep/lti-service:latest```. You can specify the tag you want.
...
```
$ docker run -p 8000:8000 itbacep/lti-service:latest
```
